...
class VQVAEMotionTrainer(nn.Module):
    ...
    def load(self, path):
        ...
        self.vqvae_model.load(pkg["model"])
    ...
    def train_step(self):
        ...
        for _ in range(self.grad_accum_every):
            ...
            gt_motion = batch["motion_full"].to(self.device)

            if self.dataset_args.enable_masking:
                gt_motion_body = self.mask_augment(
                    gt_motion_body, perc_n=0.2, perc_d=0.1
                )
                gt_motion_hand = self.mask_augment(
                    gt_motion_hand, perc_n=0.2, perc_d=0.1
                )
            mask = batch.get("motion_mask", None)

            vqvae_output = self.vqvae_model(batch)
            ...
            # usage = []
            # if len(vqvae_output.indices.shape) < 3:
            #     vqvae_output.indices = vqvae_output.indices[None, ...]
            # for indices in vqvae_output.indices.permute(2, 0, 1):
            #     used_indices = indices.flatten().tolist()
            #     usage.append(
            #         len(set(used_indices)) / self.vqvae_args.codebook_size_body
            #     )
