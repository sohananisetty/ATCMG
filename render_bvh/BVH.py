import re
import numpy as np
from render_bvh.common.quaternion import *
from render_bvh.Animation import Animation
...
def write_bvh(
    parent, offset, rotation, rot_position, names, frametime, order, path, endsite=None
):
    file = open(path, "w")
    ...
    def write(self, filename, rot, r_pos, order, offset=None, names=None, repr="quat"):
        ...
        if names is None:
            if self.names is None:
                ...
                names = self.names
        write_bvh(
            self.parents, offset, rot, r_pos, names, self.frametime, order, filename
        )
