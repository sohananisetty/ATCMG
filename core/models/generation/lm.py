...
class LMModel(StreamingModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        motion_tokenizer_params = MotionTokenizerParams(card)
        self.card = motion_tokenizer_params.num_tokens
    ...
    def forward(
        ...
        if isinstance(self.project_input, nn.Identity):
            ...
            input_ = torch.cat([self.emb[k](sequence[:, k]) for k in range(K)], -1)
        ...
        input_, cross_attention_input = self._prepare_inputs(
            (input_, sequence_mask), condition_tensors
        )
        ...
        **kwargs
        ...
        try:
            if conditions.get("text", None) is not None:
                ...
                possible_num_samples.append(batch_size)
                if (
...
class MotionGen(nn.Module):
    def __init__(self, tranformer_config, fuse_config, pattern_config):
        super().__init__()
    ...
    def forward(
        self,
        inputs: tp.Dict[str, ConditionType],
        conditions: tp.Dict[str, ConditionType],
